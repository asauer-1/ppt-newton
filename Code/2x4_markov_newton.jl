...
#Basis d=4
s4 = Array{Complex{Float64}}(undef, 4, 4, 16)
for i in 0:3, j in 1:4
    s4[:,:,i*4+j] =  kron(s2[:,:,i+1],s2[:,:,j])
end

#Basis 2x4
k64 = Array{Complex{Float64}}(undef, 8, 8, 64)

for i in 0:3, j in 1:16
    k64[:,:,16*i+j] = kron(s2[:,:,i+1],s4[:,:,j])
end

#generate density matrix from vector a
function mstate(a)
    state = Array{Complex{Float64}}(I/8,8,8)
    for i in 1:63
        mul!(state, a[i],  k64[:,:,i+1], 1, 1)
    end
    return state
end
...
function checknewt(a)
    ...
    s7 = real(dot(st3',st4))
    s8 = real(dot(st4',st4))
    return (newt2(s2) && newt3(s2,s3) && newt4(s2,s3,s4) && newt5(s2,s3,s4,s5) && newt6(s2,s3,s4,s5,s6) && newt7(s2,s3,s4,s5,s6,s7) && newt8(s2,s3,s4,s5,s6,s7,s8))
end


#flip signs according to the partial transpose
#0:15=1*X; 16:31=sigma_x*X; 32:47=sigma_y*X; 48:63=sigma_z*X
function flip(a)
    return [a[i]*(-1)^(i in 32:47) for i in 1:63]
end
...
#test_run(10000)
inf_run(20000)
#inf_run(100000, "./results_2x4.txt")